...
def mock_mt5_client():
    ...
    client.place_order = Mock(return_value={"success": True, "order_id": 12345})
    client.get_account_info = Mock(return_value={"balance": 10000.0, "equity": 10000.0})
    return client
...
def executor(mock_mt5_client, tmp_path):
    ...
    return TradeIdeaExecutor(mt5_client=mock_mt5_client, log_dir=str(tmp_path))
...
class TestValidation:
    ...
    async def test_validate_daily_loss_limit_fails(self, executor, valid_trade_idea):
        ...
        with patch(
            "backend.ai.executor.risk_limits",
            return_value={"daily_loss_limit_r": "500"},
        ):
            with patch("backend.app._calculate_daily_pnl", return_value=-600.0):
                result = await executor.validate_execution_safety(
                    valid_trade_idea, 10000.0
                )
                assert result.valid is False
...
class TestPositionSizing:
    ...
    def test_position_size_respects_limits(
        self, executor, valid_trade_idea, mock_mt5_client
    ):
        """Test position size respects symbol limits."""
        mock_mt5_client.get_symbol_info = Mock(
            return_value={"volume_min": 0.1, "volume_max": 1.0, "volume_step": 0.1}
        )
    ...
    def test_position_size_fallback_on_error(
        self, executor, valid_trade_idea, mock_mt5_client
    ):
...
class TestExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_valid_idea(
        self, executor, valid_trade_idea, mock_mt5_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_short_idea(
        self, executor, valid_trade_idea, mock_mt5_client
    ):
    ...
    async def test_execute_mt5_error_returns_failure(
        ...
        mock_mt5_client.place_order = Mock(
...
class TestLogging:
    ...
    @pytest.mark.asyncio
    async def test_failed_execution_logged(
        self, executor, valid_trade_idea, tmp_path, mock_mt5_client
    ):
