...
def test_close_position_feature():
    ...
    try:
        ...
        for pos in initial_positions:
            print(f"     - Ticket: {pos.get('ticket')}, Symbol: {pos.get('symbol')}, "
    ...
    try:
        response = requests.post(
            f"{API_BASE}/api/positions/{position_ticket}/close",
            headers=get_headers()
        )
        response.raise_for_status()
    ...
    try:
        ...
        if response.status_code == 503 or response.status_code == 200:
            ...
            print(f"     - Response: {result.get('error', {}).get('message', 'Position not found')}")
        else:
