...
class AIEngine:
    ...
    def __init__(
        ...
        self.timezone = pytz.timezone(
            self.settings.get("timezone", "Africa/Johannesburg")
        )
        logger.info(
            f"AI Engine initialized with config_dir={config_dir}, data_dir={data_dir}"
        )

    def evaluate(
        self, symbol: str, timeframe: str = "H1", force: bool = False
    ) -> Optional[TradeIdea]:
        """
        ...
        try:
            ...
            rules = load_rules(str(self.config_dir / "strategies"), symbol, timeframe)
            if not rules:
                logger.warning(f"No EMNR rules found for {symbol} {timeframe}")
                return None

            # Load symbol profile
            profile = load_profile(str(self.config_dir / "profiles"), symbol)
            if not profile:
            ...
            if not bars or len(bars) < 50:
                logger.warning(
                    f"Insufficient bars for {symbol} {timeframe}: {len(bars) if bars else 0}"
                )
                return None
            ...
            sl_price, tp_price, rr_ratio = self._calculate_sl_tp(
                current_price, direction, indicators, profile, rules
            )
            ...
            trade_idea = TradeIdea(
                ...
                indicators=IndicatorValues(
                    **{
                        ...
                        if k in IndicatorValues.__annotations__
                    }
                ),
            ...
            logger.info(
                f"Trade idea generated for {symbol}: confidence={confidence}, action={execution_plan['action']}"
            )
            return trade_idea
    ...
    def _fetch_bars(
        ...
        try:
            ...
            bars_data = self.mt5_client.copy_rates_from_pos(
                symbol, tf_constant, 0, count
            )
            if not bars_data:
                return None
                ...
                bars.append(
                    {
                        ...
                        "open": (
                            bar.get("open", 0.0) if isinstance(bar, dict) else bar[1]
                        ),
                        "high": (
                            bar.get("high", 0.0) if isinstance(bar, dict) else bar[2]
                        ),
                        "low": bar.get("low", 0.0) if isinstance(bar, dict) else bar[3],
                        "close": (
                            bar.get("close", 0.0) if isinstance(bar, dict) else bar[4]
                        ),
                        "volume": (
    ...
    def _check_alignment(self, symbol: str, timeframe: str, profile, rules) -> bool:
        ...
        try:
            ...
            if (
                current_session not in profile.bestSessions
                and current_session not in rules.sessions
            ):
                return False
    ...
    def _calculate_sl_tp(
        self, entry_price: float, direction: str, indicators: Dict, profile, rules
        ...
        try:
            ...
            if direction == "long":
                ...
                tp_price = entry_price + (
                    atr * atr_multiplier * profile.style.get("rrTarget", 2.0)
                )
            else:  # short
                sl_price = entry_price + (atr * atr_multiplier)
                tp_price = entry_price - (
