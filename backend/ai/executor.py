...
class TradeIdeaExecutor:
    ...
    def _init_execution_log(self):
        ...
        if not os.path.exists(self.execution_log_path):
            ...
            with open(self.execution_log_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    'timestamp', 'idea_id', 'symbol', 'direction', 'confidence',
                    'entry_price', 'stop_loss', 'take_profit', 'volume',
                    'rr_ratio', 'risk_pct', 'order_id', 'success', 'error'
                ])
    ...
    async def validate_execution_safety(
        ...
        if idea.status != "approved":
            errors.append(f"Trade idea must be approved (current status: {idea.status})")
        ...
        if idea.confidence < 75:
            errors.append(f"Confidence {idea.confidence} below minimum 75 for execution")
        ...
        try:
            ...
            if daily_limit > 0:
                ...
                if current_pnl <= -abs(daily_limit):
                    errors.append(f"Daily loss limit reached: {current_pnl:.2f} <= -{daily_limit:.2f}")
        ...
        try:
            ...
            if risk_pct <= 0 or risk_pct > 5:  # Max 5% risk per trade
                errors.append(f"Risk percentage {risk_pct}% outside acceptable range (0-5%)")
        except ValueError:
    ...
    def calculate_position_size(
        ...
        try:
            ...
            logger.info(f"Calculated position size: {volume} lots (risk: {risk_pct}%, amount: ${risk_amount:.2f})")
            return volume
    ...
    async def execute_trade_idea(
        ...
        try:
            ...
            if result.get('success'):
                ...
                logger.info(f"Trade idea {idea.id} executed successfully: order {order_id}")
                self._log_execution(idea, success=True, order_id=order_id, volume=volume)
            ...
            logger.error(f"Trade idea {idea.id} execution exception: {e}", exc_info=True)
            self._log_execution(idea, success=False, error=error_msg)
            return ExecutionResult(success=False, error=error_msg)
