...
class AutonomyLoop:
    ...
    def start(self, interval_minutes: int = 15) -> Dict[str, any]:
        ...
        try:
            ...
            logger.info(f"Autonomy loop started with {interval_minutes} minute interval")
    ...
    def _evaluate_all_symbols(self):
        ...
        try:
            ...
            logger.info(f"Starting autonomy loop evaluation for {len(enabled_symbols)} symbols")
            ...
            for symbol in enabled_symbols.keys():
                try:
                    ...
                    if result.get("success"):
                        ...
                        error_count += 1
                        logger.warning(f"Evaluation failed for {symbol}: {result.get('message', 'Unknown error')}")
