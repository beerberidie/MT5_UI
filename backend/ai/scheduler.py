...
def schedule_action(
    confidence: int, min_rr_ok: bool, risk_cap_pct: float = 0.03
) -> Dict[str, str]:
    """
    ...
    if confidence < 75:
        ...
        return {"action": "pending_only", "riskPct": f"{reduced_risk:.3f}"}
    ...
    return {"action": "open_or_scale", "riskPct": f"{risk_cap_pct:.3f}"}
