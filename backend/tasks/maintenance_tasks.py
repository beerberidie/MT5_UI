...
def cleanup_old_logs(self, days_to_keep: int = 30):
    ...
    try:
        ...
        logger.info(f"Log cleanup complete: {files_deleted} files deleted, "
...
def cleanup_cache(self, days_to_keep: int = 7):
    ...
    try:
        ...
        logger.info(f"Cache cleanup complete: {files_deleted} files deleted, "
                   f"{bytes_freed / 1024 / 1024:.2f} MB freed")
...
def system_health_check(self):
    ...
    try:
        ...
        try:
            ...
            if percent_free < 10:
                health_status["errors"].append(f"Low disk space: {percent_free:.1f}% free")
                health_status["status"] = "unhealthy"
            elif percent_free < 20:
                health_status["warnings"].append(f"Disk space warning: {percent_free:.1f}% free")
        ...
        try:
            ...
            if logs_dir.exists():
                ...
                if size_mb > 1000:  # 1 GB
                    health_status["warnings"].append(f"Large logs directory: {size_mb:.1f} MB")
                    if health_status["status"] == "healthy":
                        health_status["status"] = "warning"
        except Exception as e:
        ...
        try:
            ...
            if cache_dir.exists():
                ...
                if size_mb > 500:  # 500 MB
                    health_status["warnings"].append(f"Large cache directory: {size_mb:.1f} MB")
                    if health_status["status"] == "healthy":
                        health_status["status"] = "warning"
        except Exception as e:
