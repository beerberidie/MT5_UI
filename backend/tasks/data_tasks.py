...
def collect_market_data(self):
    ...
    try:
        ...
        logger.info(
            f"Market data collection complete: {results['symbols_collected']} symbols"
        )
...
def update_economic_calendar(self):
    ...
    try:
        ...
        if not calendar_api or not calendar_api.get("enabled"):
            ...
            return {"success": False, "message": "Economic calendar API not configured"}
...
def collect_rss_news(self):
    ...
    try:
        ...
        logger.info(
            f"RSS news collection complete: {results['articles_collected']} articles "
...
def _store_calendar_cache(storage: FileStorage, events: List[Dict[str, Any]]):
    ...
    try:
        ...
        with open(filename, "w") as f:
            json.dump(
                {"updated_at": datetime.utcnow().isoformat(), "events": events},
                f,
                indent=2,
            )
...
def _store_news_cache(storage: FileStorage, articles: List[Dict[str, Any]]):
    ...
    try:
        ...
        with open(filename, "w") as f:
            json.dump(
                {"updated_at": datetime.utcnow().isoformat(), "articles": all_articles},
                f,
                indent=2,
            )
