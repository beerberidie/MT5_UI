...
class APIIntegrationCreate(BaseModel):
    ...
    type: str = Field(
        ..., description="Integration type: economic_calendar, news, custom"
    )
...
class AppearanceSettings(BaseModel):
    ...
    density: str = Field(
        default="normal", description="UI density: compact, normal, comfortable"
    )
...
async def _test_api_integration(
    integration_type: str, api_key: str, base_url: Optional[str] = None
) -> APIIntegrationTestResponse:
    """
    ...
    try:
        ...
        if integration_type == "economic_calendar":
            ...
            if response.status_code == 200:
                return APIIntegrationTestResponse(
                    ...
                    response_data={
                        "status": "connected",
                        "articles_count": len(response.json()),
                    },
                )
            else:
                return APIIntegrationTestResponse(
            ...
            response = requests.get(
                base_url, headers={"Authorization": f"Bearer {api_key}"}, timeout=10
            )
        ...
        return APIIntegrationTestResponse(
            success=False, connected=False, error="Connection timeout"
        )
    except requests.exceptions.ConnectionError:
        return APIIntegrationTestResponse(
            success=False, connected=False, error="Connection failed"
        )
    except Exception as e:
        return APIIntegrationTestResponse(
            success=False, connected=False, error=f"Test failed: {str(e)}"
        )


async def _test_mt5_connection(
    ...
    try:
        ...
        if not authorized:
            ...
            mt5.shutdown()
            return AccountTestResponse(
                success=False, connected=False, error=f"Login failed: {error_code}"
            )
        ...
        if account_info is None:
            mt5.shutdown()
            return AccountTestResponse(
                success=False, connected=False, error="Failed to get account info"
            )
        ...
        return AccountTestResponse(success=True, connected=True, account_info=info_dict)

    except Exception as e:
        return AccountTestResponse(
            success=False, connected=False, error=f"Connection test failed: {str(e)}"
        )
...
async def activate_account(account_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to activate account: {str(e)}"
        )
...
async def get_integrations():
    ...
    try:
        ...
        masked_integrations = [
            _mask_integration_api_key(integ) for integ in integrations
        ]

        return {"integrations": masked_integrations}
    except Exception as e:
        raise HTTPException(
...
async def update_integration(integration_id: str, updates: APIIntegrationUpdate):
    ...
    try:
        ...
        updated_integration = await storage.update_api_integration(
...
@router.post(
    "/integrations/{integration_id}/test", response_model=APIIntegrationTestResponse
)
async def test_integration_connection(integration_id: str):
    ...
    try:
        ...
        if result.connected:
            await storage.update_api_integration(
                integration_id,
                {"last_tested": datetime.utcnow().isoformat(), "status": "active"},
            )
        else:
            await storage.update_api_integration(integration_id, {"status": "error"})
...
async def get_appearance_settings():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get appearance settings: {str(e)}"
        )
...
async def update_appearance_settings(settings: AppearanceSettings):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update appearance settings: {str(e)}"
        )
