...
class ApprovalRequest(BaseModel):
    ...
    manual_overrides: Optional[Dict[str, Any]] = Field(
        None, description="Manual parameter overrides"
    )
...
async def get_trade_ideas(
    ...
    try:
        ...
        if symbol:
            trade_ideas = [
                ti for ti in trade_ideas if ti.get("symbol") == symbol.upper()
            ]

        if approval_status:
            trade_ideas = [
                ti for ti in trade_ideas if ti.get("approval_status") == approval_status
            ]

        # Count by approval status
        pending_count = sum(
            1 for ti in trade_ideas if ti.get("approval_status") in [None, "pending"]
        )
        approved_count = sum(
            1 for ti in trade_ideas if ti.get("approval_status") == "approved"
        )
        rejected_count = sum(
            1 for ti in trade_ideas if ti.get("approval_status") == "rejected"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving trade ideas: {str(e)}"
        )
...
async def get_trade_idea(trade_idea_id: str):
    ...
    if not trade_idea:
        raise HTTPException(
            status_code=404, detail=f"Trade idea not found: {trade_idea_id}"
        )
...
async def approve_trade_idea(request: ApprovalRequest = Body(...)):
    ...
    try:
        ...
        if not trade_idea:
            raise HTTPException(
                status_code=404, detail=f"Trade idea not found: {request.trade_idea_id}"
            )
        ...
        logger.info(
            f"Trade idea {request.trade_idea_id} approved by {request.approved_by}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error approving trade idea: {str(e)}"
        )
...
async def reject_trade_idea(request: RejectionRequest = Body(...)):
    ...
    try:
        ...
        logger.info(
            f"Trade idea {request.trade_idea_id} rejected by {request.rejected_by}: {request.rejection_reason}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error rejecting trade idea: {str(e)}"
        )
...
async def modify_trade_idea(request: ModifyRequest = Body(...)):
    ...
    try:
        ...
        trade_idea["manual_overrides"]["modified_at"] = (
            datetime.utcnow().isoformat() + "Z"
        )

        # Recalculate R:R ratio if prices were modified
        if (
            "entry_price" in modifications
            or "stop_loss" in modifications
            or "take_profit" in modifications
        ):
        ...
        raise HTTPException(
            status_code=500, detail=f"Error modifying trade idea: {str(e)}"
        )
...
async def cancel_trade_idea(trade_idea_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error cancelling trade idea: {str(e)}"
        )
