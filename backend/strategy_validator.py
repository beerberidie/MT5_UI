...
def validate_strategy(strategy_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if "symbol" in strategy_data:
        ...
        if not symbol.isupper() or not symbol.isalnum():
            errors.append(
                "Symbol must be uppercase alphanumeric (e.g., EURUSD, XAUUSD)"
            )
...
def validate_condition_syntax(condition: str) -> Tuple[bool, Optional[str]]:
    ...
    valid_patterns = [
        ...
        "price_above_ema_slow",
        "price_below_ema_slow",
        "price_close_gt_ema_slow",
        "price_close_lt_ema_slow",
        # RSI conditions
        "rsi_gt_70",
        "rsi_lt_30",
        "rsi_between_40_60",
    ...
    if condition.replace("_", "").isalnum():
        logger.warning(
            f"Unknown condition '{condition}' - may not be recognized by EMNR evaluator"
        )
...
def validate_and_prepare(
    strategy_data: Dict[str, Any], user: str = "user"
