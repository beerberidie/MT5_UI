...
def _load_trade_ideas(date_from: Optional[datetime] = None, date_to: Optional[datetime] = None) -> List[Dict[str, Any]]:
    """
...
def _load_evaluations(date_from: Optional[datetime] = None, date_to: Optional[datetime] = None) -> List[Dict[str, Any]]:
    """
    ...
    for json_file in evaluations_dir.glob("evaluation_*.json"):
        try:
            ...
            if "evaluations" in eval_data and isinstance(eval_data["evaluations"], list):
...
def _load_health_checks(date_from: Optional[datetime] = None, date_to: Optional[datetime] = None) -> List[Dict[str, Any]]:
    """
...
async def get_decision_history(
    ...
    source: Optional[str] = Query(None, description="Filter by source (trade_idea, evaluation, health_check)"),
    ...
    try:
        ...
        if date_from:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid date_from format: {e}")
        
        if date_to:
            try:
                date_to_dt = datetime.fromisoformat(date_to.replace('Z', '+00:00'))
            except Exception as e:
                raise HTTPException(status_code=400, detail=f"Invalid date_to format: {e}")
        ...
        if not source or source == "trade_idea":
            ...
            all_items.extend([_convert_to_decision_item(idea, "trade_idea") for idea in trade_ideas])
        
        if not source or source == "evaluation":
            evaluations = _load_evaluations(date_from_dt, date_to_dt)
            all_items.extend([_convert_to_decision_item(eval_data, "evaluation") for eval_data in evaluations])
        
        if not source or source == "health_check":
            health_checks = _load_health_checks(date_from_dt, date_to_dt)
            all_items.extend([_convert_to_decision_item(health, "health_check") for health in health_checks])
...
async def get_decision_stats(
    ...
    try:
        ...
        all_items = (
            ...
            [_convert_to_decision_item(eval_data, "evaluation") for eval_data in evaluations] +
            [_convert_to_decision_item(health, "health_check") for health in health_checks]
        )
