        ...
        if not dry_run:
            for account in accounts:
                try:
                    ...
                    if existing:
                        print(f"   ⚠️  Account '{account['name']}' already exists, skipping")
                    else:
        ...
        if not dry_run:
            for integration in integrations:
                try:
                    ...
                    if existing:
                        print(f"   ⚠️  Integration '{integration['name']}' already exists, skipping")
                    else:
        ...
        if not dry_run:
            for feed in feeds:
                try:
                    ...
                    error_msg = f"Failed to migrate feed '{feed.get('name', 'unknown')}': {e}"
                    stats["errors"].append(error_msg)
                    print(f"   ❌ {error_msg}")
        else:
...
async def verify_migration() -> dict:
    ...
    if len(file_accounts) == len(db_accounts):
        ...
        results["differences"].append(f"Account count mismatch: File={len(file_accounts)}, DB={len(db_accounts)}")
        print(f"   ❌ Account count mismatch: File={len(file_accounts)}, DB={len(db_accounts)}")
    ...
    if len(file_integrations) == len(db_integrations):
        ...
        results["differences"].append(f"Integration count mismatch: File={len(file_integrations)}, DB={len(db_integrations)}")
        print(f"   ❌ Integration count mismatch: File={len(file_integrations)}, DB={len(db_integrations)}")
    ...
    if len(file_feeds) == len(db_feeds):
        ...
        results["differences"].append(f"Feed count mismatch: File={len(file_feeds)}, DB={len(db_feeds)}")
