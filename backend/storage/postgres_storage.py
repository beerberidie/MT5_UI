...
from backend.database import (
    Strategy, RiskConfig, SnapshotMarket, SnapshotIndicators,
    SnapshotCalendar, SnapshotNews, SnapshotAccount,
    TradeIdea, DecisionHistory, User,
    TradeIdeaStatus, DecisionAction
)
        ...
        async with get_db_context() as session:
            ...
            if existing:
                ...
                await session.execute(
                    update(RiskConfig).values(**config)
                )
            else:
                # Create new
            ...
            result = await session.execute(
                select(TradeIdea).where(
                    TradeIdea.status.in_([
                        TradeIdeaStatus.WAITING,
                        TradeIdeaStatus.NEEDS_APPROVAL
                    ])
            ...
            return [
                {
                    ...
                    "entry_price": float(idea.entry_price) if idea.entry_price else None,
